...
class ReactionNetwork:
    ...
    def reaction_blocks(self) -> list[tuple[list[dict], list[Topology]]]:
        ...
        *molecular proximity*: any two molecules within the bimolecular cutoff
        ...
        #
        # Two reactions share a block iff they share an atom, so the relation is
        # already generated by the atoms themselves: union each reaction with the
        # first reaction seen on each of its atoms and the transitive closure is
        # the same partition.  Testing all pairs instead is O(R^2) in a quantity
        # that grows with the box -- 3888 reactions on a 64-water box is 7.6M
        # frozenset intersections, a third of the whole force call -- where this
        # is O(sum of reaction sizes).
        #
        # `find` is inlined and `i`'s own root carried through the inner loop:
        # at a few atoms per reaction this runs tens of thousands of times per
        # force call, where the call overhead alone was the bulk of it.  Carrying
        # the root is safe because `parent[i]` is only ever written during
        # iteration `i` -- every write attaches an existing root to another, and
        # both are indices already seen.
        parent = list(range(len(reactions)))

        first_on_atom: dict[int, int] = {}
        for i, rxn in enumerate(reactions):
            root = i
            for atom in rxn["mapping"].values():
                owner = first_on_atom.setdefault(atom, i)
                if owner == i:
                    continue
